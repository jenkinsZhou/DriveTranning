package com.tourcoo.training.widget.citypicker.citywheel;
...
public class CityParseHelper {
     ...
     * 初始化数据，解析json数据
     */
    public void initData(List<IndustryCategory> industryCategoryList) {
       /* //todo
        String cityJson = "utils.getJson(context, .CITY_DATA)";
...
//        mProvinceBeanArrayList = new Gson().fromJson(cityJson, type);
        ...
        mProvinceBeanArrayList = parseProvinceList(industryCategoryList);
        if (mProvinceBeanArrayList.isEmpty()) {
            return;
        }
...
//            mProvinceBeenArray[p] = ;
    ...
    private ArrayList<ProvinceBean> parseProvinceList(List<IndustryCategory> industryCategoryList) {
        ArrayList<ProvinceBean> provinceBeanArrayList = new ArrayList<>();
        for (IndustryCategory industryCategory : industryCategoryList) {
            ProvinceBean provinceBean = transformIndustryCategory(industryCategory);
    ...
    private ProvinceBean transformIndustryCategory(IndustryCategory industryCategory) {
        if (industryCategory == null) {
            return null;
        }
        ProvinceBean provinceBean = new ProvinceBean();
        provinceBean.setId(industryCategory.getID());
        provinceBean.setName(industryCategory.getName());
        if (industryCategory.getIndustryCategorys() != null) {
            ArrayList<CityBean> cityList = new ArrayList<>();
            provinceBean.setCityList(cityList);
            for (IndustryCategory category : industryCategory.getIndustryCategorys()) {
                CityBean cityBean = new CityBean();
                cityBean.setId(category.getID());
                cityBean.setName(category.getName());
