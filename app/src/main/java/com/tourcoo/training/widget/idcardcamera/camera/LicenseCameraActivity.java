package com.tourcoo.training.widget.idcardcamera.camera;
 ...
 * 拍照界面
 */
public class LicenseCameraActivity extends Activity implements View.OnClickListener {
     ...
     * 行驶证识别
     * */
    public final static int TYPE_DRIVE_LICENCE = 5;
    /**
     * vin码识别
     * */
    public final static int TYPE_VIN_IDENTIFY = 6;
    ...
    public static void openCertificateCamera(Activity activity, int type) {
        Intent intent = new Intent(activity, LicenseCameraActivity.class);
    ...
    private LicenseCameraPreview licenseCameraPreview;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (type == TYPE_COMPANY_PORTRAIT) { //竖版营业执照
        ...
        } else if (type == TYPE_COMPANY_LANDSCAPE) {//横版营业执照
            ...
            containerView.setLayoutParams(containerParams);
            cropView.setLayoutParams(cropParams);
        } else if(type == TYPE_VIN_IDENTIFY){ //vin码识别

            float width = (int) (screenMinSize * 0.9);
            float height = (int) (width*0.2);
            LinearLayout.LayoutParams containerParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, (int) height);
            LinearLayout.LayoutParams cropParams = new LinearLayout.LayoutParams((int) width, (int) height);
        ...
        }else  if(type == TYPE_DRIVE_LICENCE){ //行驶证识别
            float width = (int) (screenMinSize * 0.9);
            float height = (int) (width*0.75);
            LinearLayout.LayoutParams containerParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, (int) height);
            LinearLayout.LayoutParams cropParams = new LinearLayout.LayoutParams((int) width, (int) height);
            containerParams.gravity= Gravity.CENTER;
            containerView.setLayoutParams(containerParams);
        ...
        licenseCameraPreview.setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.camera_surface) {
            ...
            licenseCameraPreview.setEnabled(true);
            resultView.setVisibility(View.GONE);
            licenseCameraPreview.startPreview();
        }
    }

    private void takePhoto() {
        optionView.setVisibility(View.GONE);
        licenseCameraPreview.setEnabled(false);
        licenseCameraPreview.takePhoto(new Camera.PictureCallback() {
            @Override
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            if (type == TYPE_COMPANY_PORTRAIT || type==TYPE_DRIVE_LICENCE || type==TYPE_VIN_IDENTIFY) {
                                left = (float) cropView.getLeft() / (float) licenseCameraPreview.getWidth();
                                top = ((float) containerView.getTop() - (float) licenseCameraPreview.getTop()) / (float) licenseCameraPreview.getHeight();
                                right = (float) cropView.getRight() / (float) licenseCameraPreview.getWidth();
                                bottom = (float) containerView.getBottom() / (float) licenseCameraPreview.getHeight();
                            } else {
                                left = ((float) containerView.getLeft() - (float) licenseCameraPreview.getLeft()) / (float) licenseCameraPreview.getWidth();
                                top = (float) cropView.getTop() / (float) licenseCameraPreview.getHeight();
                                right = (float) containerView.getRight() / (float) licenseCameraPreview.getWidth();
                                bottom = (float) cropView.getBottom() / (float) licenseCameraPreview.getHeight();
                            }
                            //裁剪及保存到文件
